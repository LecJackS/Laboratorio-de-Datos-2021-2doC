...
Pensado para ir siguiendo [estos videos](https://www.youtube.com/playlist?list=PLmKafz8lJ_AXM7uxhasLR665ZqWlpC6RD)
...
ingredientes <- ingredientes$drinks$strIngredient1
ingredientes
```
...
num.tragos <- 10
tragos     <- c()
num.ingred <- rep(NA, num.tragos)

cat("Pidiendo tragos a la api...\n> ")
for (i in c()){ #1:num.tragos){ # TODO: Remover c() y volver a 1:num.tragos
  cat(i, ", ", sep="")
  azar <- fromJSON("http://www.thecocktaildb.com/api/json/v1/1/random.php")
  tragos <- c(tragos, azar)
  
  # grep('Ingredient', colnames(al.azar))
  # startsWith(names(azar$drinks), "strIng")

  nom.ingred <- azar$drinks[, names(azar$drinks)[startsWith(names(azar$drinks), "strIngredient")]]
  nom.ingred <- nom.ingred[, !is.na(nom.ingred)]
  num.ingred[i] <- length(nom.ingred)
  
  instrucciones <- instrucciones[, !is.na(instrucciones)]
}
cat("\n> Promedio ingredientes por trago:", mean(num.ingred))
```
...
```{r}
ingrediente <- ingredientes[sample(length(ingredientes), 1)]
# Espacios por guiones, importante al insertarla en url en el siguiente paso
ingrediente <- gsub(" ", "-", ingrediente)
ingrediente <- tolower(ingrediente)
cat("> Ingrediente:", ingrediente)
```


```{r}
#cat("Pidiendo datos a url:", paste0("http://www.thecocktaildb.com/api/json/v1/1/filter.php?i=", ingrediente), "\n")

#datos_trago = fromJSON(paste0("http://www.thecocktaildb.com/api/json/v1/1/filter.php?i=", ingrediente))

#cat("Numero de tragos que utilizan ingrediente \'", ingrediente, "\': ", nrow(datos_trago$drinks), sep="")
```
...
for (t in tragos){
  #instr <- t$drinks[, names(t$drinks)[startsWith(names(t$drinks), "strInstructions")]]
  names(t$drinks)
...
```{r}
data = fromJSON("https://api.worldbank.org/v2/es/country/all/indicator/?format=json&per_page=20000")
...
m.nombres <- museos$results$nombre
m.direcciones <- museos$results$direccion

#nom.dir <- data.frame(nombre=nombres, dir=direcciones)

#table(museos$results$nombre, direccion=museos$results$direccion)

# Separar el ultimo de cada chunk despues de la ultima coma: Congreso 141, San Miguel de Tucumán, provincia de Tucumán.  

for (i in 1:length(nombres)){
  cat("Analizando museo:", m.nombres[i], "\n")
  cat("Dirección:", m.direcciones[i], "\n")
  
  dir.all <- unlist(strsplit(m.direcciones[i],", "))
  
  calle <- dir.all[1]
  provincia <- dir.all[length(dir.all)]
  
  # print(calle)
  # print(provincia)
  
  x <- fromJSON(URLencode(paste0("https://apis.datos.gob.ar/georef/api/direcciones?direccion=", calle, "&provincia=", provincia )))
  
  cat("> se ubica en \n  > lat:", x$direcciones$ubicacion$lat, "\n  > lon:", x$direcciones$ubicacion$lon, "\n\n")
