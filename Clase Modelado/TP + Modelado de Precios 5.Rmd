...
### Carga de Libreiras, Lectura del dataset y su estructura
...
Videos 1 a 10: <https://www.youtube.com/watch?v=oZh0zNwmq_4&list=PLN2e9R_DoC0THQgbage3DkS_7tCsqTd-3&index=1>
...
> Convierta el script de optimización aleatoria (Luus-Jaakola) en una función, y aplique esa función para hallar los valores óptimos de parámetros de los modelos M1.
>
> Requiere ver hasta el video 7 inclusive (53 minutos).

### Optimizacion Aleatoria

```{r}
set.seed(42)
optim.aleatoria <- function(loss.fun=Eval, mu_0=100000, alfa_0=1000, beta_0=1000){
  ...
  mejor.eval <- loss.fun(mu_0, alfa_0, beta_0) # el mejor valor
  mejores <- matrix(c(mejor.eval, mu_0, alfa_0, beta_0, 1), 1, 5)
  ...
  while (facred.acu > toler)
  {
    ...
    mu <-
      runif(1,
            mejor.mu - rango.mu * facred.acu,
            mejor.mu + rango.mu * facred.acu)
    alfa <-
      runif(1,
            mejor.alfa - rango.alfa * facred.acu,
            mejor.alfa + rango.alfa * facred.acu)
    beta <-
      runif(1,
            mejor.beta - rango.beta * facred.acu,
            mejor.beta + rango.beta * facred.acu)
    
    # Evaluacion de los nuevos valores
    valor <- loss.fun(mu, alfa, beta)
    if (valor < mejor.eval)
      # Si encuentro algo mejor -> Actualizacion
    {
      ...
      mu.hist <- c(mu, mu.hist)
      alfa.hist <- c(alfa, alfa.hist)
      beta.hist <- c(beta, beta.hist)
  ...
  all.hist <- data.frame(mejor.eval, mu.hist, alfa.hist, beta.hist)
  return (list(mejores, all.hist))
}

# mejores == c(mejor.eval, mu, alfa, beta, iteracion_k)
resultado <- optim.aleatoria(loss.fun = Eval)
mejores <- resultado[[1]]
historial <- resultado[[2]]
head(mejores)
```

```{r}
mu.hist <- historial[, 2]
alfa.hist <- historial[, 3]
beta.hist <- historial[, 4]

for(i in 1:dim(mejores)[1]){
  #cat("iter:",i,"\n")
  mejor.eval <- mejores[i, 1]
  mejor.mu <- mejores[i, 2]
  mejor.alfa <- mejores[i, 3]
  mejor.beta <- mejores[i, 4]
  

  k <- historial[i, 5]
  
  actu <- k # TODO: Recheck this
  ...
  points(alfa.hist, beta.hist, cex=0.1,col="blue")
  points(mejores[,3], mejores[,4], type='l')
...
#lines(evol2[subconj,4],col="green")
title("Evolucion de las Iteraciones")
#
...
for (b in 1:B)
{
    ...
    alfas[b] <- coe[2]
    betas[b] <- coe[3]
    R2s[b] <- summary(ajus.boot)[[9]]
}
resul <- cbind(mues, alfas, betas, R2s)
head(resul)
```
...
segments(863, 0, 863, 2000, col = "green", lty = 1,
    lwd = 3)

segments(0, 591, 2000, 591, col = "green", lty = 1,
    lwd = 3)
# cuantiles bootstrap
segments(quantile(alfas, 0.05), 0, quantile(alfas,
    0.05), 2000, col = "red", lty = 3, lwd = 3)
segments(quantile(alfas, 0.5), 0, quantile(alfas, 0.5),
    2000, col = "red", lty = 3, lwd = 3)
segments(quantile(alfas, 0.95), 0, quantile(alfas,
    0.95), 2000, col = "red", lty = 3, lwd = 3)
segments(0, quantile(betas, 0.05), 2000, quantile(betas,
    0.05), 2000, col = "red", lty = 3, lwd = 3)
segments(0, quantile(betas, 0.5), 2000, quantile(betas,
    0.5), 2000, col = "red", lty = 3, lwd = 3)
segments(0, quantile(betas, 0.95), 2000, quantile(betas,
    0.95), 2000, col = "red", lty = 3, lwd = 3)
...
datos1e <- datos1a %>% 
            ...
            filter(l1 == "Argentina", 
                   ...
                   # operaciones de venta
                   operation_type == "Venta") %>% 
            dplyr::select(id, l3, surface_total, surface_covered, price,lat,lon) %>% mutate(Precio=price,Sup=surface_covered,Fondo=surface_total-surface_covered) %>% dplyr::select(Sup,Fondo,Precio,lat,lon) %>%  filter(Fondo>=0) %>% na.omit()
datos1e
summary(datos1e)
ggpairs(datos1e)
...
```{r}
ajus.Fondo<-lm(Fondo~Sup,data=datos1d)
