...
```{r}
# Usando el parametro de html_table
correlativas <- html_table(elemento_tabla, header=1)
correlativas
```
...
Al *scrapear* estamos haciendo uso de recursos que generan o almacenan terceras personas y que pueden no querer compartirlo. Es por eso que es importante leer bien los términos y condiciones de uso del sitio del cual queremos extraer información. Por ejemplo Amazon prohibe este tipo de prácticas expresamente: ![](./amazon.png)
...
```{r}
# Listo posibles simbolos en las velocidades que no sean numericos
vel_str <- velocidades$`Velocidadkm/h`
todos_los_simbolos <- c()
for(vel in vel_str){
  vel_non_num <- gsub('[0-9]', '', vel)
  todos_los_simbolos <- c(todos_los_simbolos, vel_non_num)
}
todos_los_simbolos <- unique(todos_los_simbolos)
todos_los_simbolos
```
```{r}
vel_str <- velocidades$`Velocidadkm/h`

vel_sep <- strsplit(vel_str, '-|−') # Divide al string donde haya - o − *
# me quedo con el primero de cada valor
vel_numero <- c()
for(vel in vel_sep){
  vel_numero <- c(vel_numero, vel[1])
}
...
```{r}
superficies <- as.data.frame(superficies)
superficies.df <- data.frame("id"=superficies$`#`, Country=superficies$Country)

for(col in colnames(superficies)){
  if (col != "#" && col != "Country" && col != "% of world landmass"){
    col.as.num <- as.numeric(gsub(',', '', superficies[, col]))
    superficies.df <- cbind(superficies.df, col.as.num)
    colnames(superficies.df)[colnames(superficies.df) == "col.as.num"] <- col
  }
}

#% of world landmass

porc.tierra <- superficies$`% of world landmass`

superficies.df$`Proportion of world landmass` <- as.numeric(gsub(' %', '', porc.tierra)) / 100
superficies.df
```
...
```{r}
superficies.df$Country[superficies.df$`Tot. Area (Km²)` == max(superficies.df$`Tot. Area (Km²)`)]
```
...
```{r}
superficies.df["Water area"] <- superficies.df$`Tot. Area (Km²)` - superficies.df$`Land Area (Km²)`

# Algunos Water area son negativos, podría arreglarlo poniendolos en cero pero los dejo como advertencia
# De que existe una diferencia en los datos 

superficies.df$Country[superficies.df$`Water area` == max(superficies.df$`Water area`)]
...
```{r}
# Water / Land porque al reves, si no tiene agua, puedo dividir por cero
superficies.df["Prop. Water/Land"] <- superficies.df$`Water area`/ superficies.df$`Land Area (Km²)`

# El vaticano tiene area 0, asi que en la division genero un NaN. Lo resuelvo
superficies.df$`Prop. Water/Land`[is.nan(superficies.df$`Prop. Water/Land`)] <- 0.0

superficies.df$Country[superficies.df$`Prop. Water/Land` == max(superficies.df$`Prop. Water/Land`)]
#plot(superficies.df$`Prop. Water/Land`)
```
...
```{r}
primeros_digitos <- substr(superficies$`Tot. Area (Km²)`, 1, 1)
count <- rep(0, 10)
for(d in 0:9){
  count[d+1] <- sum(primeros_digitos == d)
}
digitos.df <- data.frame(digito=0:9, num_paises=count)
digitos.df
```
...
```{r}
digit.hist <- hist(as.numeric(primeros_digitos), breaks=-1:9, freq=F, main="Histograma de primeros dígitos", c="steelblue", xlab="dígito", labels=as.character(0:9), ylim=c(0, 0.05 + max(digitos.df$num_paises) / sum(digitos.df$num_paises)), xaxt='n')
axis(side=1, at=digit.hist$mids, labels=seq(0, 9, 1))
```
...
Si lo formateamos un poco mas prolijo vemos una parte de la respuesta que llegó: ![](./json.png)
...
```{r}
tragos = c()
print("Pidiendo tragos a la api...")
for (i in 1:10){
  print(i)
