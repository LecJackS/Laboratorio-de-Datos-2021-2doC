...
# Si quisiera solo las que tengan alguna letra entre ellas
str_view(stringr::words, "^w.+e$", match = TRUE) 
...
ggplot() + 
  (words_DE %>% arrange(by=desc(Freq)) %>% # Ordenamos en forma descendiente
             mutate(rank=row_number()) %>% # Agregamos una columna con el orden
             geom_point(shape=21, mapping=aes(x=log(rank), y=log(Freq)), color='red', fill='white')) +
  (words_IT %>% arrange(by=desc(Freq)) %>% # Ordenamos en forma descendiente
             mutate(rank=row_number()) %>% # Agregamos una columna con el orden
             geom_point(shape=21, mapping=aes(x=log(rank), y=log(Freq)), color='blue', fill='white')) + 
  (words_EN %>% arrange(by=desc(Freq)) %>% # Ordenamos en forma descendiente
             mutate(rank=row_number()) %>% # Agregamos una columna con el orden
             geom_point(shape=21, mapping=aes(x=log(rank), y=log(Freq)), color='orange', fill='white') )
...
```
```{r}
words_tfidf %>% select(word, n) %>% map(inst_EN[1])
```
